...
def calculate_vol_index(hist_value,lookback=30):
    volatility= risk.calc_volatility(hist_value, lookback)
    periods_per_year = math.floor(365.0/float(lookback))
    vix = 100*math.sqrt(periods_per_year)*volatility
    print "30 day annualized Ether VIX using daily returns (%): {}".format(vix)
    return 
